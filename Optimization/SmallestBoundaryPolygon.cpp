...
// Helper functions
//
double SmallestBoundaryPolygonProblem::distanceFromLine(Point lp1, Point lp2, Point p) {
	// https://en.wikipedia.org/wiki/Distance_from_a_point_to_a_line
	return ((lp2.y - lp1.y)*p.x - (lp2.x - lp1.x)*p.y + lp2.x*lp1.y - lp2.y*lp1.x) / sqrt(pow(lp2.y - lp1.y, 2) + pow(lp2.x - lp1.x, 2));
}


double SmallestBoundaryPolygonProblem::outerDistanceToBoundary(vector<Point> solution) {
	double sum_min_distances = 0;

	for (unsigned int pi = 0; pi < points.size(); pi++) {
		double min_dist;
		for (unsigned int li = 0; li < solution.size(); li++) {
			double act_dist = distanceFromLine(solution[li], solution[(li + 1) % solution.size()], points[pi]);
...
double SmallestBoundaryPolygonProblem::lengthOfBoundary(vector<Point> solution) {
	double sum_length = 0;
...
// Objective functions
//
double SmallestBoundaryPolygonProblem::objective(std::vector<Point> solution)
{
	return lengthOfBoundary(solution);
}

double SmallestBoundaryPolygonProblem::constraint(std::vector<Point> solution)
{
